...
**2.2 Set directory and load data.**
```{r}
...
**2.3 Check and remove missing data by row.**
```{r}
colSums(is.na(data))
data <- na.exclude(data)
```

**2.4 Pick specific bottles (closure time).**  
*This code selects measurements from bottle 113 to bottle 118.*
```{r}
sub <- data %>% filter(Bottle >= 113 & Bottle <= 118)
```

**2.5 Model and calculate standard deviation.**
```{r}
...
**2.6 Plot regression line and observations.**
```{r}
...
**2.7. Remove observations as necessary and refit model with sub1. If removal is not necessary, use the rest of script with sub instead of sub1.**  
...
**2.8 Add fitted values to dataframe.**
```{r}
sub1$CH4fitted <- fitted(m1)
```

**2.9 Calculate and add fitted diff to dataframe.**
```{r}
sub1$CH4diff <- sub1$CH4fitted - 
  lag(sub1$CH4fitted, default = first(sub1$CH4fitted))
```

**2.10 Flux calculation.**  
...
**2.11 Remove NaN and false zeros.**
```{r}
...
**2.12 Write down average flux for chosen bottles.**  
